...
console.log("\n── 2 · Price lookups ──");
// DEALER rates (from VR PRICE LIST NEW.xlsx Sheet7)
ok("P4 outdoor dealer rate = 4250", PQM.priceForPitch(outdoor, 4).rate === 4250);
ok("P2.5 outdoor gst = 18", PQM.priceForPitch(outdoor, 2.5).gst === 18);
const interp = PQM.priceForPitch(outdoor, 3.5);
ok("interp P3.5 = 4875 (between 5500 & 4250)", interp.rate === 4875, "got " + interp.rate);
...
ok("LED cost = dealer rate × area", flat.lines[0].amount === Math.round(4250 * flat.areaSqft));
